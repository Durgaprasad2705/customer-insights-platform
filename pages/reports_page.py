...
def render(df, current_user: dict) -> None:
    ...
    # Reports must still be downloadable when optional insight generation has a
    # problem.  The current analytics engine is local, but this also protects a
    # future API-backed version from making the whole Reports page unavailable.
    try:
        insights = generate_ai_insights(df)
    except Exception as exc:
        insights = []
        st.warning(f"AI insights could not be generated for this report: {exc}")
    ...
    with col1:
        ...
        if st.button("⚡  Generate PDF", use_container_width=True, key="gen_pdf"):
            with st.spinner("Building PDF report…"):
                try:
                    # Keep the generated bytes in session state.  Rendering a
                    # download button only inside st.button is fragile because
                    # Streamlit reruns the script after interactions.
                    st.session_state["report_pdf_bytes"] = generate_pdf_report(df, insights)
                except Exception as exc:
                    st.error(f"PDF generation failed: {exc}")

        if pdf_bytes := st.session_state.get("report_pdf_bytes"):
            st.download_button(
                "📥 Download PDF",
                data=pdf_bytes,
