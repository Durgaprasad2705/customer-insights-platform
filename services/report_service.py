...
def _find_col(df: pd.DataFrame, *candidates: str) -> str | None:
    """Resolve standard and reasonably named uploaded-data columns."""
...
def generate_pdf_report(df: pd.DataFrame, insights: list[dict]) -> bytes:
    ...
    # ── Pre-compute KPIs ─────────────────────────────────────────────────────
    amt_col   = _find_col(df, "TotalAmount", "Revenue", "Amount", "Sales")
    cust_col  = _find_col(df, "CustomerID", "Customer")
    cat_col   = _find_col(df, "Category", "Segment")
    brand_col = _find_col(df, "Brand", "Manufacturer")
    date_col  = _find_col(df, "PurchaseDate", "OrderDate", "Date")
    region_col = _find_col(df, "Region", "Location", "State")
    prod_col  = _find_col(df, "ProductName", "Product", "Item")
    pay_col   = _find_col(df, "PaymentMethod", "Payment")
    rating_col = _find_col(df, "CustomerRating", "Rating")

    # Uploaded CSV/Excel values may arrive as strings.  Use a local copy so
    # every KPI, table and Plotly chart has a numeric revenue field.
    df = df.copy()
    if amt_col:
    ...
    show_cols = [c for c in
                 ...
                 if c in df.columns]
    # A report must work with arbitrary uploaded schemas too.  When none of
    # the canonical fields exists, preview the first available columns instead
    # of dividing by zero while building the ReportLab table.
    if not show_cols:
        show_cols = list(df.columns[:7])
